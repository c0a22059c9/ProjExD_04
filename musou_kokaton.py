...
def main():
    ...
    while True:
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                if event.key == pg.K_SPACE:
                    ...
                    if bird_state == "normal" and score.score >= 0:
        ...
        if len(pg.sprite.spritecollide(bird, bombs, True)) != 0 and bird_state == "hyper":
            bird.change_img(8, screen)  # こうかとん悲しみエフェクト
            score.update(screen)
            pg.display.update()
            time.sleep(2)
            return
        elif len(pg.sprite.spritecollide(bird, bombs, True)) != 0 and bird_state == "hyper":
